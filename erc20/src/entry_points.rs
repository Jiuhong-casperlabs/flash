...
// EIP3156
/// Returns the `loan_fee` entry point.
pub fn loan_fee() -> EntryPoint {
    EntryPoint::new(
        String::from(LOAN_FEE_ENTRY_POINT_NAME),
        Vec::new(),
        U256::cl_type(),
        EntryPointAccess::Public,
        EntryPointType::Contract,
...
/// Returns the `max_flash_loan` entry point.
...
/// Returns the `flash_fee` entry point.
...
/// Returns the `flash_loan` entry point.
...
pub fn default() -> EntryPoints {
    ...
    // EIP3156 start
    entry_points.add_entry_point(loan_fee());
